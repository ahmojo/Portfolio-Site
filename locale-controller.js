...
(function(){
  ...
  const STATIC_COPY = [
    ...
    {selector:'.projs .proj:nth-child(1) .proj-desc', html:true, de:'Hackathon-Prototyp für kleine Betriebe. Eine Webcam prüft mit <b style="color:var(--acc)">YOLOv11n-cls</b> eingerichtete Regalplätze. Fehlt ein Produkt bei mehreren Scans, kann FastAPI eine Bestellmail mit CSV-Anhang senden.', en:'A hackathon prototype for small businesses. A webcam checks configured shelf positions with <b style="color:var(--acc)">YOLOv11n-cls</b>. If a product is missing across repeated scans, FastAPI can send a reorder email with a CSV attachment.'},
    ...
    {selector:'.projs .proj:nth-child(2) .proj-desc', html:true, de:'<b style="color:var(--acc)">cct</b> überträgt lokale Codex- und Claude-Code-Sitzungen zwischen Rechnern. Es bündelt sie als <code>.codexbundle</code>, prüft die Prüfsumme und importiert sie am Ziel. Standardmäßig braucht es keinen Cloud-Dienst.', en:'<b style="color:var(--acc)">cct</b> moves local Codex and Claude Code sessions between machines. It packages them as a <code>.codexbundle</code>, verifies the checksum and imports them at the destination. It needs no cloud service by default.'},
    ...
    {selector:'.projs .proj:nth-child(3) .proj-desc', html:true, de:'Meine Portfolio-Seite mit eigenem <b style="color:var(--acc)">FastAPI</b>-Backend. Das Backend liefert Inhalte, GitHub-Daten und Statuswerte. Ein geschützter Admin-Bereich verwaltet die Texte. Docker betreibt die Anwendung auf einer Oracle-Cloud-VM.', en:'My portfolio site with its own <b style="color:var(--acc)">FastAPI</b> backend. The backend serves content, GitHub data and status values. A protected admin area manages the text. Docker runs the app on an Oracle Cloud VM.'},
    ...
    {selector:'.projs .proj:nth-child(4) .proj-desc', html:true, de:'Lernprojekt aus dem Boot.dev-Kurs. Ein Python-Programm sendet Aufgaben an Gemini und stellt vier lokale Werkzeuge bereit. Der Agent arbeitet nur im Ordner <code>./calculator</code> und kann dort Dateien lesen, ändern und Python ausführen.', en:'A learning project from the Boot.dev course. A Python program sends tasks to Gemini and provides four local tools. The agent only works in <code>./calculator</code>, where it can read, change and run Python files.'},
    ...
    {selector:'.projs .proj:nth-child(5) .proj-desc', de:'Schulprojekt zur Vorhersage mittlerer Hauswerte in Kalifornien. Drei Jupyter Notebooks dokumentieren Datenprüfung, Modelltraining und Auswertung mit scikit-learn.', en:'A school project that predicts median house values in California. Three Jupyter notebooks document data checks, model training, and evaluation with scikit-learn.'},
